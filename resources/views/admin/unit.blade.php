        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="dashboard-card overflow-hidden">
                ...
                <div class="flex justify-between items-center px-6 py-4 border-b border-white/10">
                    <h3 class="text-lg font-semibold text-white">Manage Sections/Units</h3>
                    <a href="{{ route('admin.add.section_unit.show') }}" 
                ...
                <div class="p-6">
                    @if ($unit->isEmpty())
                        <div class="text-center py-12">
                            ...
                            <p class="text-white/70 text-lg">No sections/units available</p>
                            <p class="text-white/50 text-sm mt-1">Click "Add Section/Unit" to get started.</p>
                        </div>
                    @else
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach ($unit as $sect)
                                <div class="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 overflow-hidden hover:bg-white/10 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl group">
                                    ...
                                    <div class="p-4">
                                        <div class="flex justify-between items-start mb-2">
                                            <h3 class="text-xl font-bold text-white">{{ $sect->unit_name }}</h3>
                                            <span class="text-xs font-mono text-red-400 bg-red-500/20 px-2 py-1 rounded border border-red-500/30">{{ $sect->unit_code }}</span>
                                        </div>
                                        <p class="text-white/60 text-sm line-clamp-2 mb-4">{{ $sect->unit_description }}</p>
                                        
                                        <!-- Actions -->
                                        <div class="flex justify-end gap-2 pt-3 border-t border-white/10">
                                            <a href="{{ route('admin.update.unit.show', $sect->id) }}" 
                                            ...
                                            <a href="{{ route('admin.delete.unit.show', $sect->id) }}" 
                                               ...
                                               onclick="return confirm('Delete this section/unit?')">
                                                Delete
                                            </a>
                                        </div>
                                    </div>
