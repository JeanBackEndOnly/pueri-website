    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="dashboard-card overflow-hidden">
                ...
                <form action="{{ route('admin.update.offer', $offer->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="flex flex-col md:flex-row items-center md:items-start gap-6 mb-8">
                            ...
                            <div class="flex-1 space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="offer_title" value="{{ $offer->offer_title }}" 
                                ...
                                <div>
                                    ...
                                    <input type="text" name="time_available" value="{{ $offer->time_available }}" 
                                           class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-red-500 focus:border-transparent">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-white/80 mb-1">About Offer</label>
                                    <textarea name="description" rows="5"
                                        class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-red-500 focus:border-transparent">{{ $offer->description }}</textarea>
                                </div>
                            </div>
                        </div>
