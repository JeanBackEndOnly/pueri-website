    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="dashboard-card overflow-hidden">
                ...
                <form action="{{ route('admin.add.information.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="flex-1 space-y-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <select name="unit_id" class="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-red-500 focus:border-transparent">
                                            <option value="" class="bg-gray-800">Select Section/Unit</option>
                                            @foreach ($unit as $sect)
                                                <option value="{{ $sect->id }}" class="bg-gray-800">{{ $sect->unit_name . ' (' . $sect->unit_code . ')' }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input type="text" name="email" value="{{ old('email') }}" 
                                               class="w-full bg-white/10 border border-white/20 rounded-lg px-3
                                                py-2 text-white focus:ring-2 focus:ring-red-500 focus:border-transparent">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-white/80 mb-1">Contact</label>
                                ...
                                <div>
                                    ...
                                    <textarea name="about" rows="5"
                                        class="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-red-500 focus:border-transparent">{{ old('about') }}</textarea>
                                </div>
                            </div>
                        </div>
