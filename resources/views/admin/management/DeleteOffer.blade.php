        ...
        <div class="max-w-9xl flex justify-center items-center max:px-4 sm:px-4 lg:px-4">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-3 px-7">
                ...
                <div class="max-w flex justify-center">
                    <a href="{{ route('admin.offer') }}" class="mt-2 bg-gray-600 hover:bg-gray-700 text-white px-3 py-1 rounded-lg text-sm font-medium">
                        Cancel
                    </a>
                    <form action="{{ route('admin.delete.offer', $dept->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
